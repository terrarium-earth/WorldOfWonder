...
public class WonderBlocks {
    ...
    public static final Block DANDELION_PETALS = add("dandelion_petals", new LeavesBlock(Block.Properties.create(Material.LEAVES).hardnessAndResistance(0.2F).tickRandomly().sound(SoundType.PLANT).func_226896_b_()), new Item.Properties().group(ItemGroup.DECORATIONS));
    ...
    public static final Block STEM_WALL_SIGN = add("stem_wall_sign", new WallSignBlock(Block.Properties.create(Material.WOOD).doesNotBlockMovement().hardnessAndResistance(1.0F).sound(SoundType.WOOD).lootFrom(STEM_SIGN), WonderWoodType.STEM), null, null);
    ...
    private static <T extends Block> T add(String name, T block, Item.Properties properties, BiFunction<Block, Item.Properties, BlockItem> itemConstructor) {
        ...
        if (itemConstructor != null) WonderItems.REGISTRY.register(name, () -> itemConstructor.apply(block, properties));
        return block;
    }
}
